...
function renderTeams() {
  ...
  const slot=(key,label)=>{const c=characterById.get(t[key]),owned=c&&unit(c.id).owned;return `<button class="slot${c?' filled':''}${c&&!owned?' unowned':''}${picking===key?' selected':''}" data-slot="${key}" aria-label="${label}${c?' '+c.name:' 空き'}">${c?portrait(c):'<span class="slot-icon">＋</span>'}<strong>${c?esc(displayName(c)):label}</strong>${c?`<small>${c.element} · ${c.role}${owned?'':' · 未所持'}</small>`:'<small>クリックして選ぶ</small>'}</button>`};
...
function renderPickerList(q) {
  ...
  $('#picker-list').innerHTML=list.map(c=>`<button data-pick="${c.id}" aria-label="${esc(c.name)}" class="${unit(c.id).owned?'':'missing'}">${portrait(c)}<span>${esc(displayName(c))}<small>${c.element} · ${c.role} · ${unit(c.id).owned?'所持':'未所持'}</small></span></button>`).join('')||'<span>該当する仲間はいません</span>';
}
